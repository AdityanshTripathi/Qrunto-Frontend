  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Tables & QR Codes</h1>
      ...
      {!loading && tables.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Total Tables', value: tables.length, color: 'text-blue-400' },
            { label: 'Active Tables', value: activeTables.length, color: 'text-emerald-400' },
            { label: 'Inactive Tables', value: inactiveTables.length, color: 'text-gray-400' },
          ].map((stat) => (
            <div key={stat.label} className="bg-[#1f2937]/25 border border-[#374151]/35 rounded-[16px] px-5 py-4 text-center">
      ...
      {loading ? (
        <div className="bg-[#1f2937]/10 border border-[#374151]/30 rounded-[24px] p-24 flex flex-col items-center justify-center min-h-[400px]">
          ...
          <p className="text-gray-300 font-medium">Loading tables...</p>
        </div>
      ) : tables.length === 0 ? (
        <div className="bg-[#1f2937]/20 border border-[#374151]/40 rounded-[24px] p-12 flex flex-col items-center justify-center min-h-[400px] text-center">
          <div className="w-16 h-16 bg-[#374151]/30 rounded-2xl flex items-center justify-center mb-4 text-[#9ca3af]">
            <QrCode className="w-8 h-8" />
          </div>
          <h3 className="text-lg font-bold text-gray-200">No tables yet</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {tables.map((table) => (
            ...
            >
              {/* QR Code Display */}
              <div className="bg-white/5 p-6 flex items-center justify-center border-b border-[#374151]/25 relative">
                {table.qrCodeUrl ? (
                  <div className="flex items-center justify-center">
                    <QRCodeCanvas value={table.qrCodeUrl} size={140} />
                  </div>
                ) : (
                  <div className="w-36 h-36 bg-[#374151]/30 rounded-xl flex items-center justify-center text-[#9ca3af]">
              ...
              <div className="p-4 flex flex-col flex-1">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className={`text-[10px] font-semibold ${table.isActive ? 'text-emerald-400' : 'text-gray-500'}`}>
                ...
                {table.qrCodeUrl && (
                  <a
                    ...
                    className="flex items-center gap-1 text-[10px] text-[#9ca3af] hover:text-[#FF6B35] transition-colors mb-3 truncate"
                ...
                <div className="flex items-center gap-2 mt-auto">
                  ...
                  {table.qrCodeUrl && (
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1 py-2 bg-[#374151]/30 hover:bg-[#374151] border border-[#374151]/40 rounded-xl text-gray-300 hover:text-white transition-all text-xs font-semibold"
                  ...
                  <button
                    ...
                    className="p-2 bg-[#374151]/30 hover:bg-[#374151] border border-[#374151]/40 rounded-xl text-gray-300 hover:text-white transition-all"
                    title="View QR Code"
                  >
                  ...
                  {deleteConfirmId === table.id ? (
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className="p-1.5 bg-[#374151] text-gray-300 rounded-lg transition-all"
                      >
                    ...
                    <button
                      ...
                      className="p-2 bg-red-500/5 hover:bg-red-500/10 border border-red-500/10 hover:border-red-500/30 rounded-xl text-red-400 hover:text-red-300 transition-all"
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[#1f2937] border border-[#374151]/75 rounded-[24px] shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200 z-10 overflow-hidden">
            <div className="absolute -top-10 -right-10 w-28 h-28 bg-[#FF6B35]/10 rounded-full blur-2xl pointer-events-none" />

            <div className="flex items-center justify-between pb-5 border-b border-[#374151]/40 mb-5">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <QrCode className="w-5 h-5 text-[#FF6B35]" />
                Add New Table
              </h2>
              <button onClick={() => setIsAddModalOpen(false)} className="p-1.5 bg-[#111827] border border-[#374151] rounded-xl text-gray-400 hover:text-white transition-all">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={addForm.handleSubmit(onCreateSubmit)} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full bg-[#111827]/70 border ${
                    addForm.formState.errors.tableNumber ? 'border-red-500' : 'border-[#374151] focus:ring-[#FF6B35]'
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-3 bg-[#374151] hover:bg-[#4b5563] font-semibold text-sm text-white rounded-[12px] transition-all">
                  Cancel
                </button>
      ...
      {editingTable && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[#1f2937] border border-[#374151]/75 rounded-[24px] shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200 z-10">
            <div className="flex items-center justify-between pb-5 border-b border-[#374151]/40 mb-5">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <Edit2 className="w-5 h-5 text-[#FF6B35]" />
                Edit Table {editingTable.tableNumber}
              </h2>
              <button onClick={() => setEditingTable(null)} className="p-1.5 bg-[#111827] border border-[#374151] rounded-xl text-gray-400 hover:text-white transition-all">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={editForm.handleSubmit(onEditSubmit)} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#d1d5db] mb-2">New Table Number / Name</label>
                <input
                  type="text"
                  className={`w-full bg-[#111827]/70 border ${
                    editForm.formState.errors.tableNumber ? 'border-red-500' : 'border-[#374151] focus:ring-[#FF6B35]'
                  } rounded-[12px] py-3 px-4 text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:border-transparent transition-all`}
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setEditingTable(null)} className="flex-1 py-3 bg-[#374151] hover:bg-[#4b5563] font-semibold text-sm text-white rounded-[12px] transition-all">
                  Cancel
                </button>
                <button type="submit" disabled={actionLoading} className="flex-1 py-3 bg-[#FF6B35] hover:bg-orange-600 font-semibold text-sm text-white rounded-[12px] transition-all flex items-center justify-center gap-2 disabled:opacity-60">
      ...
      {qrViewTable && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#1f2937] border border-[#374151]/75 rounded-[28px] shadow-2xl p-8 animate-in fade-in zoom-in-95 duration-200 z-10 flex flex-col items-center gap-5 max-w-sm w-full">
            <button onClick={() => setQrViewTable(null)} className="absolute top-4 right-4 p-1.5 bg-[#111827] border border-[#374151] rounded-xl text-gray-400 hover:text-white transition-all">
              <X className="w-4 h-4" />
            </button>

            <div>
              <h2 className="text-xl font-bold text-white text-center">Table {qrViewTable.tableNumber}</h2>
              <p className="text-xs text-[#9ca3af] text-center mt-1">Scan to view menu & order</p>
            </div>
            ...
            <p className="text-[11px] text-[#9ca3af] text-center max-w-[200px] break-all">
