    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#9ca3af]">Group your menu items into clear categories like Starters, Mains, and Drinks.</p>
        </div>
        <button 
      ...
      {loading ? (
        <div className="bg-[#1f2937]/10 border border-[#374151]/30 rounded-[24px] p-24 text-center backdrop-blur-md flex flex-col items-center justify-center min-h-[400px]">
          <Loader2 className="w-10 h-10 text-[#FF6B35] animate-spin mb-4" />
          <p className="text-gray-300 font-medium">Loading categories...</p>
        </div>
      ) : categories.length === 0 ? (
        <div className="bg-[#1f2937]/20 border border-[#374151]/40 rounded-[24px] p-12 text-center backdrop-blur-md flex flex-col items-center justify-center min-h-[400px]">
          <div className="w-16 h-16 bg-[#374151]/30 rounded-2xl flex items-center justify-center mb-4 text-[#9ca3af]">
            <Tags className="w-8 h-8" />
          </div>
          <h3 className="text-lg font-bold text-gray-200">No categories found</h3>
        ...
        <div className="space-y-3">
          {categories.map((cat, index) => {
            ...
            return (
              <div 
                key={cat.id} 
                className={`bg-[#1f2937]/25 border border-[#374151]/35 hover:border-[#FF6B35]/30 rounded-[16px] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all duration-200 backdrop-blur-md group ${
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex flex-col gap-1">
                    <button
                      ...
                      className="p-1 hover:bg-[#374151]/40 text-[#9ca3af] hover:text-white rounded-md transition-all disabled:opacity-30 disabled:hover:bg-transparent"
                      title="Move Up"
                    >
                  ...
                  <span className="w-8 text-center text-xs font-mono font-bold bg-[#111827]/80 border border-[#374151]/40 py-1 px-1.5 rounded-lg text-[#FF6B35]">
                    #{cat.displayOrder}
                  </span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-100 group-hover:text-white transition-colors">{cat.name}</h3>
                      <p className="text-[10px] text-[#9ca3af] font-mono mt-0.5">ID: {cat.id.substring(0, 8)}...</p>
                    </div>
                  </div>
                </div>

                {/* Right side: Status toggle & actions */}
                <div className="flex items-center justify-between sm:justify-end gap-4 border-t border-[#374151]/20 sm:border-0 pt-3 sm:pt-0">
                  {/* Status Badge */}
                  <button
                  ...
                  <div className="flex items-center gap-2">
                    {deleteConfirmId === cat.id ? (
                      <div className="flex items-center gap-1 bg-red-950/20 border border-red-500/30 rounded-xl p-1.5 animate-in fade-in zoom-in-95 duration-150">
                        <span className="text-xs text-red-400 font-semibold px-2">Delete?</span>
                        <button
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          {/* Modal Container */}
          <div className="relative w-full max-w-md bg-[#1f2937] border border-[#374151]/75 rounded-[24px] shadow-2xl p-6 md:p-8 animate-in fade-in zoom-in-95 duration-200 z-10 overflow-hidden">
            {/* Ambient background glow inside modal */}
            <div className="absolute -top-12 -right-12 w-36 h-36 bg-[#FF6B35]/10 rounded-full blur-2xl pointer-events-none"></div>

            {/* Title / Close Header */}
            <div className="flex items-center justify-between pb-6 border-b border-[#374151]/40 mb-6 relative">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 relative">
              ...
              {/* Display Order */}
              <div>
                <label className="block text-sm font-medium text-[#d1d5db] mb-2" htmlFor="displayOrder">
                ...
                <input
                  ...
                  placeholder="e.g. 1"
                  className={`w-full bg-[#111827]/60 border ${
                    errors.displayOrder ? 'border-red-500 focus:ring-red-500' : 'border-[#374151] focus:ring-[#FF6B35]'
                  } rounded-[12px] py-3 px-4 text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:border-transparent transition-all`}
              ...
              {/* Footer Actions */}
              <div className="flex gap-3 pt-4 border-t border-[#374151]/40 mt-8">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
